...
def setup_logging(
...
):  # Flag para diferenciar config de web app
    ...
    if not is_web_app or os.getenv("PYTHON_ENV") == "development":  # Só loga em arquivo localmente
        try:
            ...
            logger.info(
                f"Logging para '{logger_name}' configurado. Saída para console. Log de arquivo local (se dev): {log_file_path}"
            )
        except Exception as e:
            # Usar print para stderr em caso de falha na configuração do logger de arquivo
            print(
                f"[CRITICAL_LOGGER_ERROR] Erro ao configurar o file handler do log para '{logger_name}': {e}",
                file=sys.stderr,
            )
            logger.warning(
                f"Logging de arquivo desabilitado para '{logger_name}' devido a erro: {e}"
            )
    else:
        logger.info(
            f"Logging para '{logger_name}' configurado. Saída para console (ambiente web/Vercel)."
        )
