...
class GeminiClient:
    def __init__(self, api_key: str, model_name: str):
        ...
        try:
            ...
            self.model = genai.GenerativeModel(
                model_name=model_name, safety_settings=default_safety_settings
            )
            self.model_name = model_name
            logger.info(f"GeminiClient inicializado com sucesso para o modelo '{model_name}'.")
        except Exception as e:
            logger.critical(
                f"Exceção crítica durante a inicialização do modelo Gemini '{model_name}': {e}",
                exc_info=True,
            )
    ...
    def send_conversational_prompt(self, prompt_text: str) -> Optional[str]:
        logger.info(
            f"Enviando prompt para o modelo: {self.model_name}. Tamanho (aprox): {len(prompt_text):,} caracteres."
        )
        try:
            ...
            response = self.model.generate_content(
                contents=prompt_text, generation_config=generation_config
            )
            ...
            logger.warning(
                "Resposta do Gemini não continha conteúdo textual esperado e não foi explicitamente bloqueada."
            )
            return None  # Ou raise ValueError("Resposta inesperada da IA.")

        except ValueError as ve:  # Captura o erro de prompt bloqueado
            logger.error(
                f"Erro de valor durante chamada à API Gemini (modelo {self.model_name}): {ve}",
                exc_info=True,
            )
            raise  # Re-levanta para ser tratado pelo endpoint da API
        except Exception as e:
            logger.error(
                f"Erro inesperado durante chamada à API Gemini (modelo {self.model_name}): {e}",
                exc_info=True,
            )
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            if response.text:
                logger.info(
                    f"Teste de conexão com '{self.model_name}' bem-sucedido. Resposta recebida."
                )
                return True
            elif hasattr(response, "prompt_feedback") and response.prompt_feedback.block_reason:
                logger.error(
                    f"Teste de conexão falhou: prompt de teste bloqueado. Razão: {response.prompt_feedback.block_reason}"
                )
                # Não levanta exceção aqui, apenas retorna False, o chamador decide o que fazer.
                return False
            else:
                logger.warning(
                    f"Teste de conexão com '{self.model_name}': resposta vazia, mas sem erro de API. Considerando OK."
                )
                return True  # Alguns modelos podem retornar vazio para prompts muito simples
        except Exception as e:
            logger.error(
                f"Exceção crítica durante o teste de conexão com '{self.model_name}': {e}",
                exc_info=True,
            )
    ...
    def close(self):
        ...
        """
        logger.info(
            f"Cliente Gemini ({self.model_name}) 'fechado'. Nenhuma ação específica necessária para o SDK atual."
        )
