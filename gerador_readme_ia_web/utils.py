...
def extract_project_data_from_zip(
    ...
    max_content_length_per_file: int = 7000,
    max_total_files_to_process: int = 70,
) -> Optional[str]:
    """
    ...
    try:
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            for item in file_infos:
                ...
                if any(
                    item.filename.lower().startswith(pattern)
                    or f"/{pattern}" in item.filename.lower()
                    for pattern in ignored_dirs_patterns
                ):
                ...
                project_data_parts.append(
                    f"- {item.filename}{' (diretório)' if item.is_dir() else ''}\n"
                )
            ...
            relevant_files_for_content = sorted(
                [
                    ...
                    if not item.is_dir()
                    and (
                        os.path.splitext(item.filename)[1].lower() in code_extensions
                        or os.path.basename(item.filename).lower() in important_filenames
                    )
                    and not any(
                        item.filename.lower().startswith(pattern)
                        or f"/{pattern}" in item.filename.lower()
                ...
                key=lambda x: (
                    0 if os.path.basename(x.filename).lower() in important_filenames else 1,
                    x.filename,
                ),
            )
            ...
            for item in relevant_files_for_content:
                if files_processed_for_content >= max_total_files_to_process:
                    logger.info(
                        f"Limite de {max_total_files_to_process} arquivos para extração de conteúdo atingido."
                    )
                    project_data_parts.append(
                ...
                try:
                    ...
                    if item.file_size > 5 * 1024 * 1024:  # Pula arquivos maiores que 5MB
                        logger.warning(
                            f"Pulando arquivo '{file_name}' por ser muito grande: {item.file_size / (1024 * 1024):.2f} MB."
                        )
                        project_data_parts.append(
                    ...
                    with zf.open(item) as file_in_zip:
                        ...
                        try:
                            ...
                            try:
                                ...
                                logger.warning(
                                    f"Arquivo '{file_name}' decodificado como latin-1 após falha no UTF-8."
                                )
                            except Exception as e_decode_latin:
                                logger.error(
                                    f"Não foi possível decodificar o arquivo '{file_name}' como UTF-8 ou latin-1: {e_decode_latin}"
                                )
                    ...
                    logger.warning(
                        f"Erro ao ler ou processar o arquivo '{file_name}' dentro do ZIP: {e_file}",
                        exc_info=True,
                    )
                    project_data_parts.append(
            ...
            if not relevant_files_for_content:
                project_data_parts.append(
                    "[Nenhum arquivo de código ou configuração relevante encontrado para extrair conteúdo.]\n"
                )

            logger.info(
                f"Extração de dados do ZIP concluída. {files_processed_for_content} arquivos tiveram seu conteúdo processado."
            )
